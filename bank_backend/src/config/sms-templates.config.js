...
import { APP, TRANSACTION, VERIFICATION } from "./constants.config.js";
...
export const LARGE_TRANSACTION_SMS = {
	getMessage: (data) => {
		...
		const action = type === TRANSACTION.DIRECTION.OUT ? "sent" : "received";
...
export function validateSMSLength(message) {
	...
	return {
		...
		warning:
			segments > 1
				? `Message will be sent as ${segments} SMS segments`
				: null,
	};
}
...
export function formatPhoneE164(
	phone,
	defaultCountryCode = APP.DEFAULT_COUNTRY_CODE,
) {
