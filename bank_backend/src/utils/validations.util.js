...
import { VALIDATION, AUTH, CURRENCY, VERIFICATION, ACCOUNT_STATUS } from '../config/constants.config.js';
...
export function validatePhone(phone) {
    ...
    if (!VALIDATION.PHONE_REGEX.test(phone)) {
        throw new InvalidFormatError('phone', '9-15 digits, optionally starting with +');
    }
...
export function validateAmount(amount) {
    ...
    if (amount < CURRENCY.MIN_TRANSFER_AMOUNT) {
        throw new InvalidAmountError(amount, `minimum is $${CURRENCY.MIN_TRANSFER_AMOUNT}`);
    }

    if (amount > CURRENCY.MAX_TRANSFER_AMOUNT) {
        throw new InvalidAmountError(amount, `maximum is $${CURRENCY.MAX_TRANSFER_AMOUNT}`);
    }
...
export function validateOTP(otp) {
    ...
    if (!otpRegex.test(otp)) {
        throw new ValidationError(`OTP must be ${VERIFICATION.OTP_LENGTH} digits`);
    }
...
export function validateTransferInputs(data) {
    ...
    if (!toEmail) throw new ValidationError('Receiver email is required');
...
export function requireActive(user) {
    if (user.accountStatus !== ACCOUNT_STATUS.ACTIVE) {
        ...
        throw new InactiveAccountError(messages[user.accountStatus] || 'Account is not active');
    }
    return user;
}
...
export function requireDifferentUsers(userId1, userId2) {
    ...
    if (id1 === id2) {
        throw new ValidationError('Cannot perform this operation with yourself');
    }
}
...
export function sanitizeUser(user) {
    ...
    const { passwordHash, refreshTokens, __v, failedLoginAttempts, accountLockedUntil, ...safeUser } = plainUser;
