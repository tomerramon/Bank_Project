 ...
 */
export async function generateOTP(userId, type= 'EMAIL_VERIFICATION', expirationMinutes = 10) {
    // Validate type
    const validTypes = ['EMAIL_VERIFICATION', 'SMS_VERIFICATION', 'PASSWORD_RESET', 'TWO_FACTOR'];
...
export async function verifyOTP(userId, otpCode, type = 'EMAIL_VERIFICATION') {
    ...
    if (!isMatch) {
        ...
        console.log(`❌ Invalid OTP for user ${userId}. Attempts: ${verification.attempts + 1}/5`);
        return false;
    }
...
export async function invalidateAllOTPs(userId) {
    ...
    console.log(`🔒 Invalidated ${result.modifiedCount} OTPs for user ${userId}`);
